        ...
        return (
            <View key={String(new Date().getTime())} style={styles.item}>
                <View style={[styles.row, { paddingVertical: 15 }]}>
                    <CheckBox disabled={isCompletedList} setIsChecked={() => onComplete({index, item})} isChecked={item.completed} />
                ...
                {
                            item.completed
                            ? null :<View style={[styles.row, styles.btnView]}>
                            <Button onPress={() => onPressEdit({ index, name: item.name })} title={"Edit"} style={[styles.deleteBtn, { marginLeft: 0 }]} />
                            <Button onPress={() => onDelete({item, index})} title={"Delete"} style={styles.deleteBtn} />
                        </View>
                        }
            </View>
                ...
                <View style={styles.modalContent}>
                    <View style={styles.modal}>
                        ...
                        <View style={styles.center}>
                            ...
                            <Button
                                disabled={!taskValue}
                                onPress={() => onPressEdit({ index: taskIndexToEdit, item: data[taskIndexToEdit], taskValue })}
                                title={"Edit"} style={[styles.deleteBtn, { marginLeft: 0, width: width / 2 }]} />
                        </View></View>
                </View>
            </Modal>
        </View>
